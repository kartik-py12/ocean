...
const Header: React.FC<HeaderProps> = ({ onSearch, onNavigate, user }) => {
    ...
    <header className="absolute top-0 left-0 right-0 z-[1000] p-4">
        <div className="flex items-center justify-between bg-slate-800/50 backdrop-blur-md p-3 rounded-xl border border-slate-700">
             ...
             <div className="flex items-center space-x-4">
                {user ? (
                  <>
                    <button onClick={() => onNavigate(Page.PROFILE)} className="px-5 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg font-semibold transition-colors text-white">
                      {user.name}
                    </button>
                  ...
                  <>
                    <button onClick={() => onNavigate(Page.LOGIN)} className="px-5 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg font-semibold transition-colors text-white">Login</button>
...
export const MapPage: React.FC<MapPageProps> = ({ onReportHazard, onNavigate, user }) => {
    ...
    return (
        <div className="relative w-screen h-screen overflow-hidden bg-slate-900">
            ...
            <Header onSearch={handleSearch} onNavigate={onNavigate} user={user} />
            <button
